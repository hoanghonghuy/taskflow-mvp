...
function getRequiredDevCredential(name: 'DEV_USER_EMAIL' | 'DEV_USER_PASSWORD'): string {
  const value = process.env[name]
  if (!value || value.trim() === '') {
    throw new Error(`${name} is required when MOCK_MODE=false outside production`)
  }
  return value
}
...
async function getDevToken(): Promise<string> {
  ...
  const email = getRequiredDevCredential('DEV_USER_EMAIL')
  const password = getRequiredDevCredential('DEV_USER_PASSWORD')
