...
**Tài khoản dùng:** `user@gmail.com` / `user1234` (đã có sẵn, login trực tiếp)
...
- [~] A7. Logout — bị chặn bởi Next.js dev overlay khi click user menu; cần thử lại sau
...
- [x] D4. Complete / incomplete — đã verify mark complete + expand Completed section
- [-] D5–D11. Detail edit / subtask / comment / delete — chưa chạy sâu

## E. Board

- [x] E1. `/board` load được
- [x] E2. Console error: **duplicate React key** — ĐÃ FIX (deduplicate tasks trong BoardView + BoardColumn)
- [-] E3. Move column / drag — cần test lại sau
...
- [x] H3. Toggle complete sau khi tạo — đã verify "Completed today" + streak 1 day

## I. Countdown

- [x] I1. Empty state + "Add Countdown"
- [~] I2–I4. CRUD countdown — đã mở form Add Countdown, chưa submit/delete
...
- [x] O1. Route 404 — ĐÃ FIX (di chuyển `not-found.tsx` ra root `app/` directory)
- [x] O2. Admin route unauthorized → redirect
- [-] O3. Spam click
- [x] O4. Board: React duplicate key errors — ĐÃ FIX (deduplicate tasks)
...
1. **FIXED — Board duplicate React keys**  
   ...
   Fix: Deduplicate tasks array trong `BoardView.tsx` (line 193-197) và `BoardColumn.tsx` (line 180-181).

2. **FIXED — Custom 404 không hiện**  
   ...
   Fix: Di chuyển file `not-found.tsx` từ `app/(app)/` ra `app/` root directory.
...
5. **NOTE — Logout blocked by dev overlay**  
   Thao tác logout bị chặn bởi Next.js dev overlay/devtools trong phiên browser test này. Cần thử lại khi tắt dev overlay.
